...
`source` is only meaningful on `chiasm` entries (sibling of `chiasm`/`parts`): an optional plain-text attribution shown as a footer line at the bottom of that chiasm's structure modal (`openChiasmModal` in `js/bible-loader.js`), e.g. crediting the book/author a chiasm's structure was drawn from. Omitted entirely when not present. Manual-only like `links`/`note` — `js/usfm-outline-extractor.js` has no concept of chiasm entries at all, so regenerating a book's topics file destroys any `chiasm`/`parts`/`source` entries the same way it destroys `note`/`links`.
